...
def check_sump_sensor(api_url, warning_threshold, critical_threshold):
    try:
        ...
        data = response.json()
        average_depth = data["average_depth_cm"]

        if average_depth is None:
            print("UNKNOWN - No valid reading from sensor")
            sys.exit(3)

        if average_depth > critical_threshold:
            print(f"CRITICAL - Average distance {average_depth} cm exceeds critical threshold of {critical_threshold} cm")
            sys.exit(2)
        elif average_depth > warning_threshold:
            print(f"WARNING - Average distance {average_depth} cm exceeds warning threshold of {warning_threshold} cm")
            sys.exit(1)
        else:
            print(f"OK - Average distance {average_depth} cm within safe range")
            sys.exit(0)
